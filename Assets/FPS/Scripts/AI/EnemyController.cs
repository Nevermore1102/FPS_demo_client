...
namespace Unity.FPS.AI
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        void Start()
        {
            // 查找场景中的EnemyManager实例并赋值给m_EnemyManager
            m_EnemyManager = FindAnyObjectByType<EnemyManager>();
            // 如果未找到EnemyManager实例，通过DebugUtility处理错误
            DebugUtility.HandleErrorIfNullFindObject<EnemyManager, EnemyController>(m_EnemyManager, this);

            // 查找场景中的ActorsManager实例并赋值给m_ActorsManager
            m_ActorsManager = FindAnyObjectByType<ActorsManager>();
            // 如果未找到ActorsManager实例，通过DebugUtility处理错误
            DebugUtility.HandleErrorIfNullFindObject<ActorsManager, EnemyController>(m_ActorsManager, this);

            // 将当前敌人控制器注册到EnemyManager中
            m_EnemyManager.RegisterEnemy(this);

            // 获取当前游戏对象的Health组件并赋值给m_Health
            m_Health = GetComponent<Health>();
            // 如果未找到Health组件，通过DebugUtility处理错误
            DebugUtility.HandleErrorIfNullGetComponent<Health, EnemyController>(m_Health, this, gameObject);

            // 获取当前游戏对象的Actor组件并赋值给m_Actor
            m_Actor = GetComponent<Actor>();
            // 如果未找到Actor组件，通过DebugUtility处理错误
            DebugUtility.HandleErrorIfNullGetComponent<Actor, EnemyController>(m_Actor, this, gameObject);

            // 获取当前游戏对象的NavMeshAgent组件并赋值给NavMeshAgent属性
            NavMeshAgent = GetComponent<NavMeshAgent>();
            // 获取当前游戏对象及其子对象的所有Collider组件并赋值给m_SelfColliders
            m_SelfColliders = GetComponentsInChildren<Collider>();

            // 查找场景中的GameFlowManager实例并赋值给m_GameFlowManager
            m_GameFlowManager = FindAnyObjectByType<GameFlowManager>();
            // 如果未找到GameFlowManager实例，通过DebugUtility处理错误
